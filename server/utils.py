...
def check_url_structure(url):
    ...
    huggingface_pattern = (
        r"^https://huggingface\.co/[\w-]+/[\w-]+/blob/[\w-]+\.(safetensors|bin|ckpt)$"
    )
...
def extract_model_file_names_with_node_info(
    json_data: Union[Dict, List], is_windows: bool = False
) -> List[ModelFileWithNodeInfo]:
    file_names = []
    ...
    def recursive_search(
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                recursive_search(
                    value,
                    key == "nodes" if not in_nodes else in_nodes,
                    type_ if in_nodes and not node_type else node_type,
                )
        elif isinstance(data, list):
            for item in data:
                type_ = item.get("type") if isinstance(item, dict) else None
                recursive_search(
                    item, in_nodes, type_ if in_nodes and not node_type else node_type
                )
        elif isinstance(data, str) and "." in data:
            original_filepath = data
            normalized_filepath = (
                convert_to_windows_path(original_filepath)
                if is_windows
            ...
            if "." + original_filepath.split(".")[-1] in model_filename_extensions:
                file_names.append(
                    ModelFileWithNodeInfo(
                        filename, original_filepath, normalized_filepath
                    )
                )
...
def run_command(cmd: List[str], cwd: Optional[str] = None, bg: bool = False) -> None:
    ...
    if bg:
        ...
        threading.Thread(
            target=print_process_output, args=(process,), daemon=True
        ).start()
        return process.pid
    else:
...
def get_ckpt_names_with_node_info(
...
def normalize_model_filepaths_in_workflow_json(workflow_json: dict) -> dict:
    ...
    for ckpt_name in ckpt_names:
        workflow_json = json.dumps(workflow_json).replace(
            ckpt_name.original_filepath.replace("\\", "\\\\"),
            ckpt_name.normalized_filepath.replace("\\", "\\\\"),
        )
...
def run_command_in_project_venv(project_folder_path, command):
    if os.name == "nt":  # Check if running on Windows
        venv_activate = os.path.join(
            project_folder_path, "venv", "Scripts", "activate.bat"
        )
    else:
...
def run_command_in_project_comfyui_venv(project_folder_path, command, in_bg=False):
    venv_activate = (
        os.path.join(project_folder_path, "venv", "Scripts", "activate.bat")
        if os.name == "nt"
        else os.path.join(project_folder_path, "venv", "bin", "activate")
    )
    ...
    if os.name == "nt":
        return run_command(
            [venv_activate, "&&", "cd", comfyui_dir, "&&", command], bg=in_bg
        )
    else:
        return run_command(
            [".", venv_activate, "&&", "cd", comfyui_dir, "&&", command], bg=in_bg
...
def install_default_custom_nodes(project_folder_path, launcher_json=None):
    ...
    run_command(
        [
            ...
            os.path.join(
                project_folder_path, "comfyui", "custom_nodes", "ComfyUI-Manager"
            ),
        ]
    )
...
def setup_custom_nodes_from_snapshot(project_folder_path, launcher_json):
    ...
    for custom_node_repo_url, custom_node_repo_info in launcher_json["snapshot_json"][
        ...
        run_command(
            ["git", "clone", custom_node_repo_url, custom_node_path, "--recursive"]
        )
        ...
        requirements_files = glob.glob(
            os.path.join(custom_node_path, "*requirements*.txt")
        )
        ...
        if os.path.exists(install_script_path):
            run_command_in_project_venv(
                project_folder_path, f"python {install_script_path}"
            )
        ...
        if custom_node_name == "ComfyUI-CLIPSeg":
            clipseg_custom_node_file_path = os.path.join(
                custom_node_path, "custom_nodes", "clipseg.py"
            )
            shutil.copy(
                clipseg_custom_node_file_path,
                os.path.join(
                    project_folder_path, "comfyui", "custom_nodes", "clipseg.py"
                ),
            )
        ...
        for file_info in file_infos:
            ...
            if os.path.exists(dest_path):
                if compute_sha256_checksum(dest_path) != sha256_checksum:
                    ...
                    print(
                        f"WARNING: File '{dest_relative_path}' already exists and has a different checksum, so renaming new file to: {new_dest_path}"
                    )
                    ...
                    rename_file_in_launcher_json(
                        launcher_json, old_dest_filename, new_dest_filename
                    )
                else:
                    ...
                    try:
                        ...
                        if hostname == "civitai.com":
                            headers["Authorization"] = (
                                f"Bearer {config['credentials']['civitai']['apikey']}"
                            )
...
# MARK
def get_launcher_json_for_workflow_json(
